        ...
        for t in range(self.H):
            ...
            envDef_Temp = pow(
                max(self.qDelta[moy[t] - 1] - (r_cb[t + 1] - r_irr7[t + 1] - r_irr8[t + 1] + q_Shire - r_irr9[t + 1]),
                    0), 2) # SO: THE MINIMUM ENVIRONMENTAL FLOW == THE Q DELTA???

            gg_env = np.append(gg_env, envDef_Temp) # RENAME

            # VICTORIA FALLS ENVIRONMENTAL DEFICIT
            envDef_Temp = pow(
                max(self.MEF_VictoriaFalls[moy[t] - 1] - (r_ka[t + 1] - r_irr2[t + 1] + q_KaLat - r_irr9[t + 1]),
                    0), 2)

            # Total environmental deficit
